...
vector < pair <int, int> >  Aho::aho_corasick::search(string s){
    ...
    iter(c, s) {
        ...
        for (out_node *out = cur->out; out; out = out->next){
            ...
            pos[all[out->keyword]].second = int(c-s.begin()-out->keyword.size()+1);
        }
    }
    return pos;
}

vvi Aho::aho_corasick::find_position_in(std::string s) {
    vvi res(N);
     go_node *cur = go;
    iter(c, s) {
        while (cur && cur->next.find(*c) == cur->next.end())
        ...
        if (!cur) cur = go;
        for (out_node *out = cur->out; out; out = out->next){
            res[all[out->keyword]].push_back(c-s.begin()-out->keyword.size()+1);
        }
    }
    return res;
}